...
이번 문제는 처음에 아예 접근조차 못하겠어서 결국 모범 답안을 보고 다시 풀어 본 문제이다.

예제를 기준으로 dp를 초기화 하고 dp[moneys]들에 1의 값을 줬다.

즉, dp[2] = 1, dp[3] = 1의 값만 줬었다.   
여기에서 for i in range(money, m+1)로 접근했고 dp[i] = min(dp[i], dp[i - money] + 1) 이라는 점화식을 만들 수 있었다. (여기서 money는 for j in moneys)

구현하는 과정에서는 dp를 0으로 초기화해서 min을 제대로 구하지 못했던 에러와 점화식을 생각하는 과정에서 시간이 오래걸렸었다.
